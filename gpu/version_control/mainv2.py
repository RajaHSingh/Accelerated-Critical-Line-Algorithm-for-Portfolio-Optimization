...
#    print "listY: \n {} \n".format(listY)
    ...
    gpu_input_matrix = gpuarray.to_gpu(input_matrix.astype(np.float32))
    ...
    #get the total number of columns in the array which will be len(listY)
    num_cols = (len(listY))
...
#	old_a = gpuarray.to_gpu(input_matrix[:,i:i+1].astype(np.float32))
	a = extract_columns(gpu_input_matrix, start=i, stop=i+1)
